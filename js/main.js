...
var config={
	"pieces":[	
		{"color":"red","image":"img/red.png","specials":[
			{"color":"red","image":"img/redspecial.png","linked":"red","powers":function(i,j){b.clearLine(i);}}
		]},
		{"color":"blue","image":"img/blue.png"},
		{"color":"yellow","image":"img/yellow.png","specials":[
			{"color":"-","image":"img/color_bomb.png","linked":"yellow","powers":function(i,j){b.bombAll();}}
		]},
...
var b = new Board({},new Settings(config),settingsLevels);

b.checkPattern();

var pieceTemplate = "";
 $.get('views/piece.tpl', function(template) {pieceTemplate=template;});
 
//load board
var clicks = [];
function removePiece(a){
		
		$("#scoreme").html(b.score);	
		$("#"+a.i+"_"+a.j).hide( "explode", {direction: "down"},100,function(){$(this).remove();} )	
		$("#p_"+a.i+"_"+a.j).html("<img src='img/explosion.png'>");
		
}
	function addPiece(piece){
		var html = Mustache.render(pieceTemplate, piece);		
		$("#p_"+piece.id).hide().html(html);
		$("#p_"+piece.id).show( "drop", {direction: "up"}, 300 )		
	}
function closePiece(a){
		
		$("#scoreme").html(b.score);	
		$("#"+a.i+"_"+a.j).hide( "explode", {direction: "down"},100,function(){$(this).remove();} )	
		$("#p_"+a.i+"_"+a.j).html("<img src='img/closed.png'>");
		
}
 $.get('views/board.tpl', function(template) {

    var rendered = Mustache.render(template, b.getBoard());
    
	$('#game').html(rendered);
	
	$(document).on("invalidMode",function(a){
		alert("invalid move");
	});
	
	
	$(document).on("removePiece",function(event,a){		
		
	});
	
	$(document).on("newScore",function(event,score){					
		$('<div/>', { class:"highscore", text:score }).appendTo('.board').hide("puff",{},1000,function(){$(this).remove();});;		
	});
	

	$(document).on("addPiece",function(event,piece){				
		var html = Mustache.render(pieceTemplate, piece);		
		$("#p_"+piece.id).hide().html(html);
		$("#p_"+piece.id).show( "drop", {direction: "up"}, 300 )		
	});
	
	$(document).on("movePiece",function(a,move){				
		var html = $("#p_"+move.from.i+"_"+move.from.j).html();		
		console.log("moving from %d,%d => %d,%d",move.from.i+1,move.from.j+1,move.to.i+1,move.to.j+1);				
		$("#p_"+move.from.i+"_"+move.from.j).html("");
		html=html.replace(move.from.i+'_'+move.from.j,move.to.i+'_'+move.to.j);				
		$("#p_"+move.to.i+"_"+move.to.j).html(html).show();	
	});
	
	$(document).on("transferPiece",function(a,move){				
			var html = $("#p_"+move.from.i+"_"+move.from.j).html();				
			var html2 = $("#p_"+move.to.i+"_"+move.to.j).html();						
			html2=html2.replace(move.to.i+'_'+move.to.j,move.from.i+'_'+move.from.j);				
			$("#p_"+move.from.i+"_"+move.from.j).html(html2);							
			html=html.replace(move.from.i+'_'+move.from.j,move.to.i+'_'+move.to.j);				
			$("#p_"+move.to.i+"_"+move.to.j).html(html);	
			$("#"+move.to.i+"_"+move.to.j).show();										
	});
	
	$(".board").click(function(a){
		var c = a.target.id.split("_");		
		//just push it if not the same one
		
		if (clicks.length>0){
			if (clicks.length==2)
				if (a.target.id!=clicks[0].join("_") && a.target.id!=clicks[1].join("_"))
					clicks.push(c);	
			if (clicks.length==1)
				if (a.target.id!=clicks[0].join("_"))
					clicks.push(c);	
		}else
				clicks.push(c);	
